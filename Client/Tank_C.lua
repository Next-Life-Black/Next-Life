...
MaxFuel = 150 -- 100 is the default Max Fuel.
decreasing = 0.0005 -- per frame.
increasing = 0.35 -- per frame. (when refuelling)
price = 1.50 -- $3 is default. When changing this variable, make sure that the (new price * increasing speed) > 1, else it won't take player money. 
...
dxDrawImage (x*0.62, y*0.78,226, 223,"images/fuelcircle.png",0,0,0)
dxDrawImage (x*0.62, y*0.8,226, 223,"images/fuelarrow.png",getElementData(veh,"fuel") + 230,0,0)
dxDrawImage (x*0.77,y*0.63,300,300,"images/speedcircle.png")
dxDrawImage (x*0.77,y*0.63,300,300,"images/speedarrow.png",getElementSpeed(veh,"mph")*0.74 - 11,0,0)
dxDrawImage (x*0.87,y*0.788,50,50,"images/carhealth.png",getElementRotation(veh),0,0,tocolor(0 - getElementHealth(veh)*0.255,getElementHealth(veh)*0.255,0,255))
end
end
...
outputChatBox("Schalte bitte dein motor aus um das Tanken zu starten",0,255,0)
    function showDx()
       ...
       dxDrawText("Total Preis : "..math.floor(getElementData(localPlayer,"price")*100)/100, x*0.457, y*0.449, x*0.842, y*0.488, tocolor(255, 255, 255, 255), 1.10, "sans", "left", "center", false, false, true, false, false)
		...
		dxDrawText("Starte dein Motor um das Tanken zu stoppen", x*0.457, y*0.488, x*0.842, y*0.527, tocolor(255, 255, 255, 255), 1.10, "sans", "center", "center", false, false, true, false, false)
		end
	end
	...
	if veh then
		...
		local prices = getElementData(localPlayer,"price")
			if getVehicleEngineState(veh) == false and (not getElementData(veh,"fuel") == false) and getElementData(veh,"fuel") <= MaxFuel and getPlayerMoney(localPlayer) > 1.5 and getElementData(localPlayer,"refuelling") == true then
				...
				getPlayerMoney(localPlayer)== price
			end
	end
end
